...
const CardList = ({user}) => {
    ...
    const handleViewProfile=()=>{
      navigate(`/agent-profile/${user.username}/${user.role}`)
    }

    const handleViewAccount=()=>{
      navigate(`/agentcustaccounts/${user.username}/${user.role}`)
    }

    const handleMarketing=()=>{
      navigate(`/marketing/${user.username}/${user.role}`)
    }
    ...
    <>
        <div className="app__grid">
          ...
          >
            ...
            <Grid item xs={12} sm={6} md={3} lg={3}>
              <div className="mycard" onClick={handleMarketing}>
